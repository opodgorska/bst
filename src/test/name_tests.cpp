...
getTestAddress ()
{
  const CTxDestination dest
    = DecodeDestination ("3AKw29hsk7zFvtsGqDMcnP5EuoDSDXo8T2");
...
BOOST_AUTO_TEST_CASE (name_tx_verification)
{
  ...
  /* Greedy names.  */
  mtx.vin.clear ();
  mtx.vout[1].nValue = COIN / 10000;
  BOOST_CHECK (CheckNameTransaction (mtx, 212500, view, state, 0));
  mtx.vout[1].nValue = COIN / 10000 - 1;
  BOOST_CHECK (!CheckNameTransaction (mtx, 212500, view, state, 0));
  ...
  BOOST_CHECK (!CheckNameTransaction (mtx, 100000+(365*1440)-1, viewUpd, state, 0));
  mtx.vin.push_back (CTxIn (inUpdate));
  BOOST_CHECK (CheckNameTransaction (mtx, 100000+(365*1440)-1, viewUpd, state, 0));
  BOOST_CHECK (!CheckNameTransaction (mtx, 100000+(365*1440), viewUpd, state, 0));
  BOOST_CHECK (IsStandardTx (mtx, reason));

  /* Check update of FIRSTUPDATE output, plus expiry.  */
  mtx.vin.clear ();
  mtx.vin.push_back (CTxIn (inFirst));
  BOOST_CHECK (CheckNameTransaction (mtx, 100000+(365*1440)-1, viewFirst, state, 0));
  BOOST_CHECK (!CheckNameTransaction (mtx, 100000+(365*1440), viewFirst, state, 0));
  ...
  BOOST_CHECK (!CheckNameTransaction (mtx, 100000+(365*1440)-1, view, state, 0));
  BOOST_CHECK (CheckNameTransaction (mtx, 100000+(365*1440), view, state, 0));
...
BOOST_AUTO_TEST_CASE (name_expire_utxo)
{
  ...
  BOOST_CHECK (!data.isExpired (100000+(365*1440)-1) && data.isExpired (100000+(365*1440)));
  data.fromScript (100010, coinId2, op2);
  view.SetName (name2, data, false);
  BOOST_CHECK (!data.isExpired (100010+(365*1440)-1) && data.isExpired (100010+(365*1440)));
  ...
  BOOST_CHECK (ExpireNames (100000+(365*1440)-1, view, undo1, setExpired));
  ...
  BOOST_CHECK (ExpireNames (100000+(365*1440), view, undo1, setExpired));
  ...
  BOOST_CHECK (ExpireNames (100010+(365*1440), view, undo2, setExpired));
  ...
  BOOST_CHECK (UnexpireNames (100010+(365*1440), undo2, view, setExpired));
  BOOST_CHECK (setExpired.size () == 1 && *setExpired.begin () == name2);
  BOOST_CHECK (!view.GetCoin (coinId1, coin));
  BOOST_CHECK (view.GetCoin (coinId2, coin));
  BOOST_CHECK (coin == coin2);
  ...
  BOOST_CHECK (!UnexpireNames (100000+(365*1440)+1, undo1, view, setExpired));
  BOOST_CHECK (!UnexpireNames (100000+(365*1440)-1, undo1, view, setExpired));

  /* Undo the first expiration.  */
  BOOST_CHECK (UnexpireNames (100000+(365*1440), undo1, view, setExpired));
  BOOST_CHECK (setExpired.size () == 1 && *setExpired.begin () == name1);
  BOOST_CHECK (view.GetCoin (coinId1, coin));
  BOOST_CHECK (coin == coin1);
  BOOST_CHECK (view.GetCoin (coinId2, coin));
