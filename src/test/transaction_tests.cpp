...
BOOST_AUTO_TEST_CASE(test_IsStandard)
{
    ...
    static constexpr size_t maxDataSize=995999;//this value should be the same as in src/rpc/data.cpp
    std::string str=random_string( (maxDataSize+1)*2 );
    t.vout[0].scriptPubKey = CScript() << OP_RETURN << ParseHex(str.substr(0,maxDataSize*2).c_str());
    BOOST_CHECK_EQUAL(MAX_OP_RETURN_RELAY, t.vout[0].scriptPubKey.size());
    BOOST_CHECK(IsStandardTx(t, reason));

    // MAX_OP_RETURN_RELAY+1-byte TX_NULL_DATA (non-standard)
    t.vout[0].scriptPubKey = CScript() << OP_RETURN << ParseHex(str.c_str());
