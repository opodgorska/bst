...
const std::string strMessageMagic = "BST Signed Message:\n";
        ...
        for (const CTxIn& txin : tx.vin) {
            ...
            if (!view.HaveCoin(txin.prevout)) {
                ...
                for (size_t out = 0; out < tx.vout.size(); out++) {
                    ...
                    //bioinfo change: from namecoin - use HaveCoin() instead of HaveCoinInCache()
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // Subsidy is cut in half every 1 050 000 blocks which will occur approximately every 2 years.
    nSubsidy >>= halvings;
    return nSubsidy;
}
    ...
    if (!hashAssumeValid.IsNull()) {
        ...
        if (it != mapBlockIndex.end()) {
            if (it->second->GetAncestor(pindex->nHeight) == pindex &&
                ...
                fScriptChecks = (GetBlockProofEquivalentTime(*pindexBestHeader, *pindex, *pindexBestHeader, chainparams.GetConsensus()) <= 6 * 60 * 24 * 7 * 2);
            }
        }
    }
 ...
 * Pruning cannot take place until the longest chain is at least a certain length (100000 on mainnet, 1000 on testnet, 1835 on regtest).
